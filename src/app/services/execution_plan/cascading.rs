...
impl CascadingExecutionPlan {
    ...
    pub fn new(
        ...
        assert_eq!(accounts.len(), model_ids.len(), "Number of model IDs must match number of accounts");
        ...
        account_data.sort_by(|a, b| {
            a.4
                .cmp(&b.4)
                .then_with(|| b.2.health_score().partial_cmp(&a.2.health_score()).unwrap_or(std::cmp::Ordering::Equal))
        });
        ...
        let tiers: Vec<CascadingTier> = tiers.into_iter().take(quality_config.max_tiers as usize).collect();
    ...
    pub fn add_cost(&mut self, cost_microdollars: u64) {
        self.total_cost_microdollars = self.total_cost_microdollars.saturating_add(cost_microdollars);
    }
    ...
    pub fn execute(
        ...
        while let Some(tier) = self.current_tier() {
            ...
            let tier_accounts: Vec<PlannedAccount> = self
                .tiers
                .iter()
                .map(|t| t.account.clone())
                .collect();
