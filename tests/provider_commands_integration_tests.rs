...
fn create_test_repo() -> (JsonProviderRepository, TempDir) {
...
async fn test_cli_list_providers_formatting() {
    ...
    let result = cmd_list_providers(&repo).await;
    assert!(result.is_ok());
...
async fn test_cli_enable_disable_provider_workflow() {
    ...
    // Add provider
    let args = create_add_args("test-enable", "Test Enable", "https://api.enable.com", Some("sk-key"));
...
async fn test_cli_remove_and_readd_provider() {
    ...
    // Add provider
    let args1 = create_add_args("readd", "Readd Provider", "https://api.readd1.com", Some("sk-key1"));
    cmd_add_provider(args1, &repo).await.unwrap();
    ...
    let args2 = create_add_args("readd", "Readd Provider New", "https://api.readd2.com", Some("sk-key2"));
    cmd_add_provider(args2, &repo).await.unwrap();
...
async fn test_cli_provider_persistence_across_repo_reloads() {
    ...
    let args = create_add_args("persistent", "Persistent Provider", "https://api.persistent.com", Some("sk-key"));
...
async fn test_cli_provider_url_with_different_schemes() {
    ...
    // HTTP URL
    let args1 = create_add_args("http-provider", "HTTP Provider", "http://api.http.com", Some("sk-key"));
    cmd_add_provider(args1, &repo).await.unwrap();
    
    // HTTPS URL
    let args2 = create_add_args("https-provider", "HTTPS Provider", "https://api.https.com", Some("sk-key"));
    cmd_add_provider(args2, &repo).await.unwrap();
    
    // URL with port
    let args3 = create_add_args("port-provider", "Port Provider", "https://api.port.com:8080/v1", Some("sk-key"));
    cmd_add_provider(args3, &repo).await.unwrap();
...
async fn test_cli_provider_enable_disable_multiple_times() {
    ...
    // Add provider
    let args = create_add_args("toggle", "Toggle Provider", "https://api.toggle.com", Some("sk-key"));
    cmd_add_provider(args, &repo).await.unwrap();
...
async fn test_cli_provider_validate_disabled_provider() {
    ...
    let mut args = create_add_args("disabled-validate", "Disabled Validate", "https://api.disabled.com", Some("sk-key"));
    args.disabled = true;
    cmd_add_provider(args, &repo).await.unwrap();
...
async fn test_cli_provider_crud_complete_workflow() {
    ...
    // CREATE
    let add_args = create_add_args("crud-test", "CRUD Test Provider", "https://api.crud.com", Some("sk-key"));
