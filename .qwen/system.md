...
Use `topic_key` to update the same observation over time instead of creating duplicates:

| Topic Key Pattern | Use Case |
|------------------|----------|
| `architecture/auth-model` | Authentication architecture decisions |
| `architecture/db-schema` | Database schema evolution |
| `decision/deps-versions` | Dependency version decisions |
| `pattern/error-handling` | Error handling patterns |
| `config/release-profile` | Release profile configuration |
...
Next time you update auth architecture, use the same `topic_key` to update instead of creating a new observation.
