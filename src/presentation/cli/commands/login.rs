...
pub async fn handle_login_command() -> Result<()> {
    ...
    let account_repo: Arc<dyn AccountRepository + Send + Sync> = Arc::new(JsonAccountRepository::new()?);
    let provider_repo: Arc<dyn ProviderRepository + Send + Sync> = Arc::new(JsonProviderRepository::new()?);
    ...
    if provider_result.is_err() {
        println!("Provider '{}' not found. Please add a provider first using 'llm-router provider add'.", provider_id);
        return Ok(());
    }
    ...
    if !provider.enabled {
        println!("Provider '{}' is disabled. Please enable it first using 'llm-router provider enable'.", provider_id);
        return Ok(());
    }
    ...
    match auth_service.initiate_auth(provider_id).await {
        Ok(verifier_or_instructions) => {
            if verifier_or_instructions.is_empty() {
                ...
                match auth_service.complete_auth(provider_id, api_key.to_string()).await {
                    Ok(account) => {
