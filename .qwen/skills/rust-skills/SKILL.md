...
description: >
  ...
  testing, and common anti-patterns. Invoke with /skills rust-skills.
license: MIT
metadata:
  author: leonardomso (original), adapted for JARVIS
  version: "1.0.0"
  sources:
    - Rust API Guidelines
...
# Rust Best Practices — 179 Reglas
...
**MANDATORY: Load this skill BEFORE writing any Rust code.**
...
| `err-no-unwrap-prod` | **NUNCA** use `.unwrap()` in production code |
...
| `async-no-lock-await` | **NUNCA** hold `Mutex`/`RwLock` across `.await` |
...
When reviewing code, cite violated rules:
...
❌ Violás err-no-unwrap-prod (línea 89)
```
...
- **searching-external-documentation** — For up-to-date API docs
- **sdd-apply** — For implementation workflow
- **sdd-verify** — For verification against specs
