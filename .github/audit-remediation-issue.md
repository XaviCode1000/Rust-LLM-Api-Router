...
La auditoría estática del repositorio revela **vulnerabilidades de nivel 1** que comprometen la seguridad (credenciales expuestas en código de producción) y la estabilidad del runtime asíncrono (deadlocks potenciales bajo carga). Adicionalmente, existen **violaciones severas al patrón Clean Architecture** que deben revertirse para mantener la mantenibilidad y la dirección correcta de dependencias.

**Scope del audit:** 4356 symbols, 9302 relationships, 300 execution flows (GitNexus index)  
...
- `src/domain/entities/openai_types.rs` importa `axum::http::StatusCode` y `axum::response::{IntoResponse, Response}`
- `src/app/health.rs` importa `axum` y define rutas HTTP
...
pub struct InsecureStorage {
    data: Arc<std::sync::Mutex<HashMap<String, String>>>,  // ← std::sync::Mutex en async
}
```

`InsecureStorage` usa `std::sync::Mutex` dentro de un trait asíncrono. Aunque actualmente no mantiene el lock cruzando `.await` points, llamar `.lock().unwrap()` en código async es un anti-pattern que dificulta el razonamiento sobre concurrencia y puede volverse peligroso si el código evoluciona.
...
Todas las implementaciones `From` para `DomainError` usan `format!()` y pierden el error original:
...
- Audit report generado por análisis estático + GitNexus (4356 symbols, 9302 relationships)
