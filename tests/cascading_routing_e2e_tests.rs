...
use rust_llm_api_router::app::services::execution_plan::{
    ExecutionContext, PlanningOptions, CascadingTier, ExecutionConfig, CascadingExecutionPlan,
    ProviderPricing,
};
...
fn test_planning_options_cost_optimized_enables_budget_mode() {
    ...
    assert!(!options.enable_cascading, "Cost optimized should not enable cascading by default");
}
...
fn test_budget_mode_triggers_cascading_selection() {
    ...
    let context = ExecutionContext::new("req-1", "gpt-4")
        .with_planning_options(PlanningOptions::cascading());
...
async fn test_heuristic_evaluator_accepts_good_response() {
    ...
    // Act
    let score = evaluator.evaluate_quality(&account, response, &health).await;
...
async fn test_heuristic_evaluator_rejects_incoherent_and_short() {
    ...
    assert!(score.checks_failed.contains(&"coherence".to_string()), "Coherence check should fail");
...
async fn test_heuristic_evaluator_accepts_valid_json() {
    ...
    assert!(!score.checks_failed.contains(&"structure".to_string()), "Structure check should pass");
...
async fn test_quality_threshold_boundary_exactly_075() {
    ...
    assert!(score.checks_failed.contains(&"completeness".to_string()), "Completeness check should fail");
}
...
fn test_streaming_config_flags() {
    ...
    assert!(!streaming.enable_quality_escalation, "Streaming should disable quality escalation");
    assert!(!non_streaming.stream, "Non-streaming config should have stream = false");
    assert!(non_streaming.enable_quality_escalation, "Non-streaming should enable quality escalation");
}
...
async fn test_streaming_prevents_cascading_concept() {
    ...
    let model_ids = vec!["gpt-4".to_string(), "claude-2".to_string(), "mixtral".to_string()];
    ...
    assert!(!result.used_quality_escalation, "Quality escalation should not be used in streaming");
